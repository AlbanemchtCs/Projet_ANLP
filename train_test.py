...
def tok_dataset(dataset, tokenizer):
    ...
    # [ [1 if j == x else 0 for j in range(num_labels)]for x in dataset['Label']]
    dic['labels'] = [x for x in dataset['Label']]
...
def exec_train(model, tokenizer, dataset, pond=False):
    ...
    if pond:
        ...
        class_weights = torch.Tensor(
            [1. for i in range(nb_classes)]).to(device)
    ...
    for epoch in range(EPOCH):
        ...
        for batch in data_train:
            ...
            # prompt
            pbar.set_postfix(loss=loss_tot/nb_samples,
                             accuracy=total_correct/nb_samples)
